...
- **Pause screen has an arena selector** — "Change Arena" button shows an arena grid. Selecting an arena sets `currentArenaId` local state AND syncs to `matchSettings.arenaId` in the store (via `setMatchSettings`). Both must be updated — the local state triggers the game loop restart, the store ensures rematch/victory screen flows use the correct arena. Without the store sync, rematch would revert to the original arena.
...
- **Hitstop is per-player, not global** — `Player.hitstopTimer` freezes individual attacker+victim for ~7 frames (0.12s) on kill. `MatchState.hitstopZoom` drives a global 3% camera zoom punch. During hitstop: animation frame advance is skipped (player looks frozen), physics loop is skipped (no input/gravity/collision), but visual timers (`damageFlashTimer`, `burnTimer`) still tick. The rest of the game (other players, particles, gibs, shockwaves) continues normally. Multi-kills use `Math.max` (no stacking). `respawnPlayer()` resets `hitstopTimer`. White flash on victim uses splat-shaped ellipse matching `drawSplatCharacter` dimensions.
...
- **Game mods are runtime-only** — `GameMods` (7 booleans in `MatchSettings.mods`) are applied in the GameLoop constructor via `eff*` multipliers, per-player fields, or shallow arena copies. They must NEVER mutate base constants or arena definitions, as that would corrupt nav data. The Super Bounce mod shallow-copies the arena (`{ ...arena, bouncyPlatforms: [...] }`). Mods are persisted as JSON in `bunnybrawl_mods` localStorage key. The Carrot Chase mod also affects AI behavior — `evaluateActions` receives a `carrotChase` flag that skips combat evaluators and boosts carrot pursuit weight. The Underwater Gravity mod applies 0.6x gravity, 0.6x max fall speed, 0.9x jump impulse — stacks multiplicatively with theme physics.
...
- **Victory screen uses two-column layout** — left column: scoreboard + match stats, right column: stats table + MVP highlights. This fits within 720px viewport. If adding more sections, keep both columns balanced. The victory screen also has a "Change Arena" button that opens an arena grid overlay — selecting an arena updates the store and starts a new match immediately.
- **CHAR_EMOJI for React UI** — `CHAR_EMOJI[char.name]` from `characters.ts` is the single source of truth for character emojis. Used in React components (victory screen scoreboard, stats table, MVP highlights) with a `.row-emoji` CSS class for consistent sizing. Don't duplicate emoji mappings.
