...
- **PlayerInput abstraction** — All input sources (keyboard, rule-based AI, network remote, touch, ML policy, synthetic random) implement the `PlayerInput` interface (`{ slot, getAction(state, ctx?) → InputState }`). `Simulator` holds `Map<PlayerSlot, PlayerInput>` and that map is the single source of truth — no inline branching. The `PlayerInputContext` carries per-tick environment data (`networkInputs`, local touch `airborne`) consumed by `RemoteInput` / `TouchAdapter`. `GameLoop.setNetworkMode` swaps human-slot PlayerInputs between KeyboardInput and RemoteInput; `simulator.setTouchInput` installs a TouchAdapter. Resolution order (network > touch > AI/keyboard) is preserved by which adapter is installed.
...
- `MainMenu.test.tsx`, `VictoryScreen.test.tsx`, `switchArena.test.ts > respawns players at new arena spawn points`, and `integration.test.ts > network mode round-trip > fixedUpdate with explicit inputMap drives both players` have known pre-existing failures (logo.png import denied by Vite test transform; switchArena spawn-point assertion is flaky; integration test has P2 wrap-around at the seam). Reproduce against `HEAD~` before assuming a new regression.
- **Default to Simulator-level tests with `CapturedEvents`**, not GameLoop. New gameplay tests go in `src/engine/__tests__/simulator-gameplay.test.ts` — construct `new Simulator({ arena, settings, activePlayers, events: new CapturedEvents() })`, drive `sim.fixedUpdate(FIXED_TIMESTEP)`, assert on `sim.getState()` and `events.sfx`/`events.matchEnd`/etc. Sim tests need no audio/renderer/howler/canvas mocks and run ~10× faster than the GameLoop equivalent. The helper lives in `src/engine/__tests__/helpers/eventSink.ts`.
- **Reach for `gameLoop.test.ts` only for adapter behavior**: RAF lifecycle, debug-key handler, render-scale subscription, browser-API integration (`audio.stopAllGameSounds`), or assertions that genuinely require `loop.particleSystem`/`loop.cosmeticStep` wiring (gibs, particle pool, footstep `audio.play` mock, score-animations, shockwaves). Those tests still need the audio/renderer/howler/canvas mock prelude at the top of `gameLoop.test.ts`. Always call `loop.stop()` in `afterEach` to prevent keydown listener leaks.
