...
/** Create an empty AwarenessSnapshot for pre-allocation (used by AIController pooling). */
export function createEmptyAwareness(): AwarenessSnapshot {
  return {
    self: { x: 0, y: 0, vx: 0, vy: 0, onGround: false, score: 0, slowed: false, fat: false, invincible: false },
    nearestEnemy: null, priorityTarget: null, roamTarget: null,
    stompTarget: null, stompThreat: null,
    airborneAbove: [], nearestCarrot: null,
    nearestHazard: null, nearbyHazards: [],
    nearestPlatformAbove: null, nearestPlatformBelow: null,
    landingPlatform: null, nearEdge: false,
    inZeroG: false, inCurrent: 0, nearGeyser: null, geyserEscapeDx: 0,
    nearbyBotCount: 0, leaderScore: 0, onElevatedPlatform: false,
    currentPlatformIdx: -1, navTarget: null,
  };
}

/**
 * Fill a pre-allocated AwarenessSnapshot in-place (avoids top-level + array allocation).
 * Same logic as buildAwareness, writes results into `target`.
 */
export function buildAwarenessInto(
  target: AwarenessSnapshot,
  self: Player,
  state: MatchState,
  arena: Arena,
  awarenessRadius: number,
  ...
  const result = buildAwareness(self, state, arena, awarenessRadius, pathfindingDepth, preferSafePath, mirrorNav);

  // Copy scalars and nullable objects
  target.self.x = result.self.x; target.self.y = result.self.y;
  target.self.vx = result.self.vx; target.self.vy = result.self.vy;
  target.self.onGround = result.self.onGround; target.self.score = result.self.score;
  target.self.slowed = result.self.slowed; target.self.fat = result.self.fat;
  target.self.invincible = result.self.invincible;

  target.nearestEnemy = result.nearestEnemy;
  target.priorityTarget = result.priorityTarget;
  target.roamTarget = result.roamTarget;
  target.stompTarget = result.stompTarget;
  target.stompThreat = result.stompThreat;
  target.nearestCarrot = result.nearestCarrot;
  target.nearestHazard = result.nearestHazard;
  target.nearestPlatformAbove = result.nearestPlatformAbove;
  target.nearestPlatformBelow = result.nearestPlatformBelow;
  target.landingPlatform = result.landingPlatform;
  target.nearEdge = result.nearEdge;
  target.inZeroG = result.inZeroG;
  target.inCurrent = result.inCurrent;
  target.nearGeyser = result.nearGeyser;
  target.geyserEscapeDx = result.geyserEscapeDx;
  target.nearbyBotCount = result.nearbyBotCount;
  target.leaderScore = result.leaderScore;
  target.onElevatedPlatform = result.onElevatedPlatform;
  target.currentPlatformIdx = result.currentPlatformIdx;
  target.navTarget = result.navTarget;

  // Reuse existing arrays — clear + copy to avoid allocation
  target.airborneAbove.length = 0;
  for (let i = 0; i < result.airborneAbove.length; i++) target.airborneAbove.push(result.airborneAbove[i]);
  target.nearbyHazards.length = 0;
  for (let i = 0; i < result.nearbyHazards.length; i++) target.nearbyHazards.push(result.nearbyHazards[i]);
}
