import type { Arena, Platform, SpawnPoint, HazardZone, EffectZone, AABB, SurfaceTag, WeatherParticle, Ctx2D } from '../types';
...
export interface ArenaPack {
  ...
  // ---- Lighting (L2: per-arena static emitter catalog) ----
  /** Static emitters baked at arena-load. Dynamic emitters (carrot glow,
   *  per-player aura) live as fields on entities and are synthesized into
   *  the EmitterPipeline per frame by the renderer. */
  lights?: ReadonlyArray<import('../lighting/types').Light>;
