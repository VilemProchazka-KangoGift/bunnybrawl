...
- **`Math.fround()` covers ALL simulation-affecting arithmetic.** x86 JIT can use 80-bit extended precision; ARM uses 64-bit doubles. `physics.ts`, `gameLoop.ts`, and `stomp.ts` all declare `const f = Math.fround`. Coverage: player velocity/position (physics.ts), effect zone forces + knockback + entity positions (gameLoop.ts), spawn/game timers (`timeElapsed`, `dayPhase`, `carrotTimer`, `springSpawnTimer`, `thornSpawnTimer`, `lavaRockTimer`, geyser timers), player status timers (`hitstopTimer`, `fatTimer`, `slowTimer`, `burnTimer`, `splatTimer`, `respawnTimer`, `invincibleTimer`), ghost physics, and lava rock physics. Timer zero-crossing differences between peers cause permanent RNG desync (spawning consumes gameRandom() calls).
...
- **AI evaluators must pre-consume `rnd()` before conditional branches.** If awareness data (airborneAbove count, navTarget existence, nearbyBotCount) differs due to float precision, conditional `rnd()` calls consume different numbers of seeded PRNG values — permanently desyncing the RNG. Fix: call `rnd()` upfront and store in local variables, then use them inside branches.
