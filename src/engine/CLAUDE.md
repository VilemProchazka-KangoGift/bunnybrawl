...
- **Room code**: 4 chars from `ABCDEFGHJKMNPQRSTUVWXYZ23456789` (~923k combos). Length is exported as `ROOM_CODE_LENGTH` from `transport.ts` and imported by `OnlineModal.tsx` for maxLength / submit gating. Bumping requires updating the regex in `transport.test.ts` and the three online E2E helpers (`[A-Z2-9]{4}`).
